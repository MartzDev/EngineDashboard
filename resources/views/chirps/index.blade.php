            ...
            <ol class="inline-flex items-center space-x-1 md:space-x-3">
                ...
                <li aria-current="page">
                    <div class="flex items-center">
                        ...
                        <span
                            class="ml-1 text-sm font-medium text-gray-500 md:ml-2 dark:text-gray-400">{{ Str::title(__('chirp')) }}</span>
                    </div>
                </li>
            ...
            <div
                ...
                <div class="px-4 py-2 bg-white rounded-b-lg dark:bg-gray-800">
                    ...
                    <textarea id="message" name="message" rows="8"
                        ...
                        placeholder="{{ __('¿Que estas pensando?') }}" required>{{ old('message') }}</textarea>
                    <x-input-error :messages="$errors->get('message')" class="mt-2" />
                    @switch(session('status'))
                        @case('chirp-registrado')
                        ...
                        @case('chirp-editado')
                            <div class="mt-2 font-medium text-sm text-blue-600">
                                {{ __('chirp editado') }}
                            </div>
                        @break

                        @case('chirp-eliminado')
                            <div class="mt-2 font-medium text-sm text-red-600">
                                {{ __('chirp eliminado') }}
                            </div>
                        @break

                        @case('chirp-restaurado')
                            <div class="mt-2 font-medium text-sm text-yellow-500">
                                {{ __('chirp restaurado') }}
                            </div>
                        @break
                ...
                <div class="flex-1">
                    <div class="flex justify-between items-center">
                        <div>
                            ...
                            @if ($chirp->deleted_at)
                                <small
                                    class="ml-2 text-sm text-red-600">{{ $chirp->deleted_at->format('j M Y, g:i a') }}
                                    {{ __('Deleted') }}
                                </small>
                            @elseif (!$chirp->created_at->eq($chirp->updated_at))
                                <small
                                    class="ml-2 text-sm text-blue-600">{{ $chirp->updated_at->format('j M Y, g:i a') }}
                                    {{ __('Edited') }}</small>
                            @else
                                <small
                        ...
                        @if ($chirp->user->is(auth()->user()))
                            <x-dropdown>
                                ...
                                <x-slot name="content">
                                    ...
                                    @else
                                        <form
                                            action="{{ route('chirps.restore', ['id' => $chirp->id, 'chirp' => $chirp->slug]) }}"
                                            method="POST">
                                            @csrf
                                            @method('PUT')
                                            <x-dropdown-link :href="route('chirps.restore', [
                                                'id' => $chirp->id,
                                                'chirp' => $chirp->slug,
                                            ])"
                                                onclick="event.preventDefault(); this.closest('form').submit();">
                                                ...
                                                <span class="ml-3">{{ __('Restore') }}</span>
                                            </x-dropdown-link>
                                        </form>
                                    @endif
                                </x-slot>
