    ...
    <div class="max-w-2xl p-4 mx-auto sm:p-6 lg:p-8">
        <form method="post" action="{{ route('chirp.update', $chirp) }}">
            ...
            <textarea id="message" name="message" required placeholder="{{ __('What\'s on your mind?') }}"
                class="block w-full mb-2 border-gray-300 rounded-md shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">{{ old('message', $chirp) }}</textarea>
            <x-input-error for="message" class="mb-2" />

            <x-button class="mt-4">{{ __('editar') }}</x-button>
        </form>
    </div>
</x-app-layout>
